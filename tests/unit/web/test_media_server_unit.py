...
class TestMediaServer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.media_server = MediaServer(
            port=MEDIASERVER_PORT, data_root_dir=self.test_dir, log=None
        )
    ...
    def test_get_media_sources(self):
        ...
        self.assertTrue(
            any("album1" in path for path in self.media_server.items_dict.values())
        )
...
class TestServiceHandler(unittest.TestCase):
    ...
    def test_validate_url(self):
        """Test URL validation"""
        valid_urls = ["http://example.com/test.jpg", "https://example.com/test.jpg"]
        invalid_urls = ["ftp://example.com/test.jpg", "file:///test.jpg", "invalid_url"]
