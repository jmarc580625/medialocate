...
class TestUrlValidator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.path_test_cases = [
            ...
            ('aa/\rx\ny\tz',                True,       False,  True),  # spacing characters must be removed by urlparse
    ...
    def _check_result(self, expected, result, message, component, value):
        ...
        if expected:
            self.assertTrue(
                result, 
                f"{component} should be valid: {value}, got error: {message}"
            )
        else:
            self.assertFalse(
                result,
                f"{component} should be invalid: {value}, but was marked valid"
            )
            self.assertTrue(
                message,
                f"Error message should not be empty for invalid URL: {value}"
            )
    ...
    def test_path_validation(self):
        ...
        path_test_cases = [test_case for test_case in self.path_test_cases if test_case[2]]
    ...
    def test_query_validation(self):
        ...
        query_test_cases = [test_case for test_case in self.query_test_cases if test_case[2]]
    ...
    def test_url_validation_with_path_and_query(self):
        ...
        path_test_cases = [test_case for test_case in self.path_test_cases if test_case[3]]
        for path, path_expected_result, _, _ in path_test_cases:
            ...
            query_test_cases = [test_case for test_case in self.query_test_cases if test_case[3]]
