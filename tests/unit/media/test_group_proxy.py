...
import tempfile
import unittest
from medialocate.media.group_proxy import (
    MediaProxies,
...
class TestMediaProxies(unittest.TestCase):
    ...
    def test_proxies_matching(self):
        ...
        found = self.proxies.find_proxies(
            "other_group", self.proxy_threshold, [self.gps2, self.gps3], time.time()
        )
    ...
    def test_proxies_serialization(self):
        ...
        proxy = Proxy(self.proxy_threshold, [(self.gps1, [self.gps2, self.gps3])])
...
class TestMediaProxiesController(unittest.TestCase):
    ...
    def test_proxy_finding(self):
        ...
        controller_groups.groups = [MediaGroups.Group(self.gps1, ["file1.jpg"])]
        ...
        source_groups.groups = [MediaGroups.Group(self.gps2, ["file2.jpg"])]
        ...
        try:
            ...
            source_groups_path = os.path.join(
                source_medialocation, MEDIAGROUPS_STORE_NAME
            )
    ...
    def test_store_operations(self):
        ...
        try:
            ...
            with self.controller:
                # First find proxies
                proxy_count = self.controller.find_proxies(
                    source_dir, self.proxy_threshold
                )
                self.assertGreater(proxy_count, 0)
            ...
            self.assertEqual(
                data["label"], os.path.basename(os.path.realpath(self.temp_dir))
            )
        finally:
            shutil.rmtree(source_dir)
