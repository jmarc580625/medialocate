...
class TestProxyMediaCommand(unittest.TestCase):
    ...
    def test_proxy_media_basic(self):
        ...
        # Act
        with patch('sys.argv', ['proxy_media', '-t', self.target_dir, self.source_dir1]):
            main()

        # Assert
        # Check proxy store exists
        proxy_store = os.path.join(self.target_dir, MEDIALOCATION_DIR, MEDIAPROXIES_STORE_NAME)
    ...
    def test_proxy_media_with_custom_threshold(self):
        ...
        # Act
        with patch('sys.argv', ['proxy_media', '-d', '2.5', '-t', self.target_dir, self.source_dir1]):
            main()
    ...
    def test_proxy_media_update_no_force_flag(self):
        ...
        self.assertEqual(final_mtime, initial_mtime, "Proxy store should not have been regenerated")
    ...
    def test_proxy_media_update_with_force_flag(self):
        ...
        # force proxies update 
        with patch('sys.argv', ['proxy_media', '-f', '-t', self.target_dir, self.source_dir1]):
            main()

        # Assert
        self.assertTrue(os.path.exists(proxy_store))
        final_mtime = os.path.getmtime(proxy_store)
        self.assertGreater(final_mtime, initial_mtime, "Proxy store should have been regenerated")
        
    def test_proxy_media_with_multiple_directories(self):
        """Test proxy media with multiple source directories"""
        # Act
        with patch('sys.argv', ['proxy_media', '-t', self.target_dir, self.source_dir1, self.source_dir2]):
            main()
