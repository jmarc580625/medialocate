...
class TestFileFinder(unittest.TestCase):
    ...
    def setUp(self):
        ...
        |   |   |   |   |---test4-i.txt file: rwxrwxrwxrwx |     x     |    x   |     o    |    o   |   x    |   o  |
                                                           ...
                                                           |    17     +   12   +    14    +   15   +   8    |   4  |     <= total by filter
                                                                 V          V         V         V       V        V         """
        self.expected_files_count = {              'no filter': 17,#        V         V         V       V        V
        ...
        self.filters = {
            ...
            'prune filter' : [f'{TestFileFinder.dir_prefix}{TestFileFinder.sufix_ignore_1}', f'{TestFileFinder.dir_prefix}{TestFileFinder.sufix_ignore_4}'],
            'depth filter' : 2,
            'age filter' : 0,
            }
        ...
        if TestFileFinder.working_directory is None:
            ...
            for ext in TestFileFinder.file_extensions:
                with open(os.path.join(root_path, f'{TestFileFinder.file_prefix}{ext}'), 'w') as f:
                    f.write('')
            # |---depth1-x directory: rwx------
            dir_path = os.path.join(root_path, f'{TestFileFinder.dir_prefix}{TestFileFinder.sufix_exclude_1}')
            os.makedirs(dir_path)
            # |   |---test1-x.txt file: rwxrwxrwxrwx
            with open(os.path.join(dir_path, f'{TestFileFinder.file_prefix}{TestFileFinder.sufix_exclude_1}{TestFileFinder.ext_txt}'), 'w') as f:
                f.write('')
            # |---depth1-i directory: rwxrwxrwxrwx
            dir_path = os.path.join(root_path, f'{TestFileFinder.dir_prefix}{TestFileFinder.sufix_ignore_1}')
            os.makedirs(dir_path)
            # |   |---test1-i.txt file: rwxrwxrwxrwx
            # |   |---test1-i.py file: rwxrwxrwxrwx
            with open(os.path.join(dir_path, f'{TestFileFinder.file_prefix}{TestFileFinder.sufix_ignore_1}{TestFileFinder.ext_txt}'), 'w') as f:
                f.write('')
            with open(os.path.join(dir_path, f'{TestFileFinder.file_prefix}{TestFileFinder.sufix_ignore_1}{TestFileFinder.ext_py}'), 'w') as f:
                f.write('')
            # |---depth1-a directory: rwxrwxrwxrwx
            dir_path = os.path.join(root_path, f'{TestFileFinder.dir_prefix}{TestFileFinder.sufix_11}')
            os.makedirs(dir_path)
            # |   |---test1-a.txt file: rwxrwxrwxrwx
            # |   |---test1-a.py file: rwxrwxrwxrwx
            # |   |---test1-a.json file: rwxrwxrwxrwx
            for ext in TestFileFinder.file_extensions:
                with open(os.path.join(dir_path, f'{TestFileFinder.file_prefix}{TestFileFinder.sufix_11}{ext}'), 'w') as f:
                        f.write('')
            # |---depth1-b directory: rwxrwxrwxrwx
            dir_path = os.path.join(root_path, f'{TestFileFinder.dir_prefix}{TestFileFinder.sufix_12}')
            os.makedirs(dir_path)
            ...
            for ext in TestFileFinder.file_extensions:
                with open(os.path.join(dir_path, f'{TestFileFinder.file_prefix}{TestFileFinder.sufix_12}{ext}'), 'w') as f:
                        f.write('')
            # |   |---depth2-1-a directory: rwxrwxrwxrwx
            dir_path = os.path.join(root_path, f'{TestFileFinder.dir_prefix}{TestFileFinder.sufix_12}', f'{TestFileFinder.dir_prefix}{TestFileFinder.sufix_2}')
            os.makedirs(dir_path)
            # |   |   |---test2-1-a.txt file: rwxrwxrwxrwx
            # |   |   |---test2-1-a.py file: rwxrwxrwxrwx
            # |   |   |---test2-1-a.json file: rwxrwxrwxrwx
            for ext in TestFileFinder.file_extensions:
                with open(os.path.join(dir_path, f'{TestFileFinder.file_prefix}{TestFileFinder.sufix_2}{ext}'), 'w') as f:
                        f.write('')
            # |   |   |---depth3-2-1-b directory: rwxrwxrwxrwx
            dir_path = os.path.join(dir_path, f'{TestFileFinder.dir_prefix}{TestFileFinder.sufix_3}')
            os.makedirs(dir_path)
            # |   |   |   |---test3-2-1-b.txt file: rwxrwxrwxrwx
            with open(os.path.join(dir_path, f'{TestFileFinder.file_prefix}{TestFileFinder.sufix_3}{TestFileFinder.ext_txt}'), 'w') as f:
                f.write('')
            # |   |   |   |---depth4-i directory: rwxrwxrwxrwx
            dir_path = os.path.join(dir_path, f'{TestFileFinder.dir_prefix}{TestFileFinder.sufix_ignore_4}')
            os.makedirs(dir_path)
            # |   |   |   |   |---test4-i.txt file: rwxrwxrwxrwx
            with open(os.path.join(dir_path, f'{TestFileFinder.file_prefix}{TestFileFinder.sufix_ignore_4}{TestFileFinder.ext_txt}'), 'w') as f:
                f.write('')
    ...
    def test_get_counters_with_no_filters(self):
        ...
        self.assertEqual(counters['files'], self.expected_counters['no filter']['files'])
        self.assertEqual(counters['depth'], self.expected_counters['no filter']['depth'])
        self.assertEqual(counters['found'], self.expected_counters['no filter']['found'])
    ...
    def test_get_counters_with_all_filters(self):
        ...
        # Assert
        self.assertEqual(counters['dirs'], self.expected_counters['all filters']['dirs'])
        self.assertEqual(counters['files'], self.expected_counters['all filters']['files'])
        self.assertEqual(counters['depth'], self.expected_counters['all filters']['depth'])
        self.assertEqual(counters['found'], self.expected_counters['all filters']['found'])
