...
class TestProcessMemory(unittest.TestCase):
    ...
    def test_init_with_function_action(self):
        # Arrange
        def myAction(file, file_hash): return 0
        working_directory = "dummy_directory_name"
    ...
    def test_init_with_callable_object(self):
        # Arrange
        class MyCallable:
            def __call__(self, file, file_hash): return 0
        myAction = MyCallable()
        working_directory = "dummy_directory_name"
    ...
    def test_process_a_new_file(self):
        ...
        with patch('batch.controler.ProcessingStatus') as StatusMock:
            ...
            with patch('batch.controler.DictStore') as StoreMock:
                ...
                StatusMock.getFromStore.assert_called_once_with(StoreMock.return_value, key)
    ...
    def test_process_with_ignore_from_action(self):
        # Arrange
        def ignore_from_action(file, file_hash): return 1
        filename = "my_file.txt"
        working_directory = "dummy_directory_name"
        key = 'key'
        states = Enum('State', [("DONE", "done"), ("ONGOING", "tmp"), ("IGNORE", "ignore"), ("ERROR", "error")])
    ...
    def test_process_with_error_from_action(self):
        # Arrange
        def error_from_action(file, file_hash): return 11
        filename = "my_file.txt"
        working_directory = "dummy_directory_name"
        key = 'key'
        states = Enum('State', [("DONE", "done"), ("ONGOING", "tmp"), ("IGNORE", "ignore"), ("ERROR", "error")])
    ...
    def test_clean(self, OsMock):
        ...
        with patch('batch.controler.ProcessingStatus') as StatusMock:
            ...
            with patch('batch.controler.DictStore') as StoreMock:
                ...
                # Assert
                StatusMock.getAllFromStore.assert_called_once_with(StoreMock.return_value)
    ...
    def test_get_counters_with_combination_of_file_state_and_action_results(self, OsMock):
        ...
        with patch('batch.controler.ProcessingStatus') as StatusMock:
            ...
            StatusMock.getFromStore.side_effect = lambda x, y: mock_matrix[mock_index][0]
            StatusMock.return_value.getState.side_effect = lambda: mock_matrix[mock_index][1]
            StatusMock.return_value.getTime.return_value = now
            StatusMock.filename_hash.return_value = key
