    ...
    try:
        if regenerate_option:
            ...
            with MediaLocateAction(working_dir, output_file_name) as media_action:
                ...
                log.info(
                    f"{finder.get_counters()}".replace("'", "").replace("{", "").replace("}", "")
                )
                log.info(
...
def main():
    ...
    parser.add_argument(
        "-d", action="store_true", help="restricts file processing in the current directory"
    )
    parser.add_argument(
        "-f", action="store_true", help="forces file processing even when already being processed"
    )
    parser.add_argument(
        "-o",
        type=str,
    ...
    parser.add_argument(
        "-r",
        action="store_true",
        help="unconditionnaly regenerate the output from previous processing outcomes\nignore -d and -f options",
    )
    parser.add_argument(
        "dirs", nargs="*", help="target directories fr\ndefault target is the local directory"
    )
