...
import logging
import subprocess
from parameters import MEDIALOCATION_STORE_NAME, MEDIALOCATION_PAGE_DATA, MEDIALOCATION_RES_DIR, MEDIALOCATION_PAGE_PROLOG, MEDIALOCATION_PAGE_EPILOG
#-------------------------------------------------------------------------------
# third party modules
#-------------------------------------------------------------------------------
...
class MediaLocateAction:
    ...
    def __init__(self : 'MediaLocateAction', 
                 working_directory : str, 
                 outfile : str, 
                 parent_logger  : str = None):
        self.working_directory = working_directory
        self.out_file = outfile
        self.log = logging.getLogger(".".join(filter(None, [parent_logger, MediaLocateAction.LOGGER_NAME])))
        self.store = DictStore(self.working_directory, MediaLocateAction.STORE_NAME)
        self.store.open()
        self.ressources_path = os.path.join(os.path.dirname(__file__), MediaLocateAction.RESSOURCE_DIR_NAME)
        self.prolog_resource_path = os.path.join(self.ressources_path, MediaLocateAction.PROLOG_RESSOURCE_NAME)
        self.epilog_resource_path = os.path.join(self.ressources_path, MediaLocateAction.EPILOG_RESSOURCE_NAME)
        self.data_appendix_path = os.path.join(self.working_directory, MediaLocateAction.DATA_APPENDIX_NAME)
    ...
    def assert_package_is_installed(self : 'MediaLocateAction', params : list[str]):
        ...
        if not package_name in self.installed_packages:
            try:
                if subprocess.check_call(params, stdout = subprocess.DEVNULL, stderr = subprocess.DEVNULL) == 0:
    ...
    def thumb_from_video_with_python_av_packages(self : 'MediaLocateAction', filename : str, thumbnail_name : str) -> bool:
        #TODO: to be tested
        #import av
    ...
    def thumb_from_video_with_ffmpeg_shell(self : 'MediaLocateAction', filename : str, thumbnail_name : str) -> bool:
        self.assert_package_is_installed(['ffmpeg', '-version'])            
        return subprocess.run(["ffmpeg", "-hide_banner", "-v", "quiet", "-nostdin", "-i", filename, "-vf", "thumbnail,scale = w = 128:h = -1", "-frames:v", "1", thumbnail_name]).returncode == 0

    def thumb_from_picture_with_ffmpeg_shell(self : 'MediaLocateAction', filename : str, thumbnail_name : str) -> bool:
        self.assert_package_is_installed(['ffmpeg', '-version'])            
        #TODO: to be tested
        return subprocess.run(["ffmpeg", "-hide_banner", "-v", "quiet", "-nostdin", "-i", filename, "-vf", "thumbnail,scale = w = 128:h = -1", thumbnail_name]).returncode == 0

    def thumb_from_picture_with_python_PIL_package(self : 'MediaLocateAction', filename : str, thumbnail_name : str) -> bool:
    ...
    def thumb_from_picture_with_convert_shell(self : 'MediaLocateAction', filename : str, thumbnail_name : str) -> bool:
        self.assert_package_is_installed(['convert', '-version'])
        #TODO: to be tested
        return subprocess.run(["convert", "-quiet", "-resize", "128x", filename, thumbnail_name]).returncode == 0

    def create_thumb_from_media(self : 'MediaLocateAction', filename : str, media_type : MediaType, thumbnail_name : str) -> bool:
    ...
    def copy_location_page_appendices_and_get_associated_html_links(self : 'MediaLocateAction', file_extension : str, template : str) -> str:
        snippet = ""
        for  source in  os.listdir(self.ressources_path):
            if source.endswith(file_extension):
                ...
                if not os.path.exists(target_path) or os.path.getmtime(target_path) < os.path.getmtime(source_path):
    ...
    def create_location_page(self : 'MediaLocateAction'):
        if self.store.size() > 0 and self.store.is_touched():
            ...
            html_stylesheet_snippet = self.copy_location_page_appendices_and_get_associated_html_links(".css", stylesheet_link_template)
            script_link_template = """<script src = "{path}"></script>"""
            html_script_snippet = self.copy_location_page_appendices_and_get_associated_html_links(".js", script_link_template)

            # generates medialocation data/script appendix file if needed
            data_link_template = """<script type="text/javascript" src="{path}" class="json"></script>"""
            html_script_snippet += data_link_template.format(path=PurePath(self.data_appendix_path).as_posix())
            ...
            if not os.path.exists(self.out_file) or \
                os.path.getmtime(self.out_file) < os.path.getmtime(self.prolog_resource_path)  or \
                os.path.getmtime(self.out_file) < os.path.getmtime(self.epilog_resource_path):

                with open(self.out_file, 'w') as f:
                    with open(self.prolog_resource_path, 'r') as html_prolog:
                        html_prolog_template = html_prolog.read()
                        f.write(html_prolog_template.format(stylesheets = html_stylesheet_snippet, scripts = html_script_snippet))
