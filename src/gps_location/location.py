...
class GPS:
    ...
    def midpoint_to(self : 'GPS', gps : 'GPS'):
        self.latitude, self.longitude, gps.latitude, gps.longitude = map(radians, [self.latitude, self.longitude, gps.latitude, gps.longitude])
        dlon = gps.longitude - self.longitude

        Bx = cos(gps.latitude) * cos(dlon)
        By = cos(gps.latitude) * sin(dlon)
        lat_mid = atan2(sin(self.latitude) + sin(gps.latitude), sqrt((cos(self.latitude) + Bx) ** 2 + By ** 2))
