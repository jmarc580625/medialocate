...
def main():
    ...
    parser.add_argument('-f', action = 'store_true', help = 'forces grouping even no changes have been made in media location data')
    parser.add_argument("dirs", nargs="*", help="directories to search for media files location data")
    ...
    for directory in directories:
        ...
        if not args.f and \
            os.path.exists(output_file_name) and \
        ...
        with DictStore(working_directory, input_name) as input_store:
            ...
            with open(output_file_name, 'w') as output_file:
                ...
                log.info(f"{directory} : grouping {len(locations)} media in {len(media_groups.groups)} groups")
