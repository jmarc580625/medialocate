...
from media.parameters import MEDIALOCATION_DIR, MEDIAPROXIES_STORE_PATH, MEDIAGROUPS_STORE_NAME, MEDIAGROUPS_STORE_PATH
...
class MediaProxies:
    ...
    class Proxy:
        ...
        def __init__(self : 'MediaProxies.Proxy', proxy_threshold : float, matches : list[tuple[GPS, list[GPS]]] = [], timestamp : float = None) -> 'MediaProxies.Proxy':
        ...
        def toDict(self : 'MediaProxies.Proxy') -> dict:
            return {
                ...
                'matches' : [(gps.toDict(), [gps.toDict() for gps in gps_list]) for gps, gps_list in self.proxy_matches],
        ...
        def fromDict(self : 'MediaProxies.Proxy', d : dict) -> 'MediaProxies.Proxy':
            return MediaProxies.Proxy(
                d['proxy_threshold'], 
                matches = [(GPS.fromDict(gps), [GPS.fromDict(gps) for gps in gps_list]) for gps, gps_list in d['matches']], 
    ...
    def __init__(self : 'MediaProxies', label : str, group_locations : list[GPS] = [], proxies : dict[str, 'MediaProxies.Proxy'] = {}) -> 'MediaProxies':
        self.label = label
    ...
    def fromDict(cls : 'MediaProxies', d : dict) -> 'MediaProxies':
        return MediaProxies(
            d['label'], 
            proxies = {label : MediaProxies.Proxy.fromDict(proxy) for label , proxy in d['proxies'].items()},
        )
    

    def find_proxies(self : 'MediaProxies', label : str, proxy_threshold : float, gps_list : list[GPS], last_update : float) -> int:
        if label == self.label:
...
class MediaProxiesControler:
    ...
    def __init__(self : 'MediaProxiesControler', working_directory : str, parent_logger : str = None):
        ...
        self.log = logging.getLogger(".".join(filter(None, [parent_logger, MediaProxiesControler.LOGGER_NAME])))
    ...
    def find_proxies(self : 'MediaProxiesControler', groups_path : str, proxy_threshold : float) -> int:
        if self.proxies is None:
            ...
            try:
                ...
                with open(source_path, 'r') as input_file:
                    ...
                    if proxy_number == -1:
                        self.log.info(f"{self.proxies.label} : gps list unmodified since last proxy search")
                    elif proxy_number == -2:
                        self.log.info(f"{self.proxies.label} : no proxy search for self")
                    else:
                        self.log.info(f"{self.proxies.label} : find {proxy_number} proxy with {name} ")
            except Exception as e:
